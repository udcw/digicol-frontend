  ...
  const fetchOpportunities = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('id, title, opportunity_type, company')
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500 ml-2">({opportunities.length})</span>
          </div>
              ...
              {opportunities.length === 0 ? (
                ...
                opportunities.map((opp) => (
                  <tr key={opp.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="px-6 py-3 text-sm text-gray-500">{opp.id}</td>
                    <td className="px-6 py-3 text-sm font-medium text-slate-800">{opp.title}</td>
                    <td className="px-6 py-3 text-sm text-gray-500">{opp.opportunity_type}</td>
                    <td className="px-6 py-3 text-sm text-gray-500">{opp.company || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
